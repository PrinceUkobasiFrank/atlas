import type { AppData, Note, Question, Source, View } from './types';
import { VIEW_LABELS, STATUS_LABELS } from './constants';
...
function debounce<T extends (value: string) => void>(fn: T, delay: number): (value: string) => void {
...
function buildDashboard(container: HTMLElement): void {
  ...
  const onFocusChange = debounce((value: string) => {
    data = updateFocus(data, value);
    saveData(data);
  }, 300);

  focusInput.addEventListener('input', (ev: Event) => {
    const target = ev.target as HTMLInputElement;
    onFocusChange(target.value);
  });
